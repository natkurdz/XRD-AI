...
class Data_file:
    ...
    def find_counts_of(self,peaks):
        self.two_theta_in_counts = peaks
...
def delete_aparature(counts,counts_bac,x,ap_counts,ap_x,ap_step):
    ...
    return end_counts
@njit
def find_counts_of_peaks(two_theta,theta_start,step):
    '''function finding peak index in range of x where peak from two theta extsts'''
...
# a = Data_file(0)
# a.read_data_of_hkl()
# a.find_counts_of(find_counts_of_peaks(a.two_theta,a.theta_start,a.step))
# a.without_ap(delete_aparature(a.counts,a.counts_bac,a.x,ap.counts,ap.x,ap.step))
# a.without_aparature_counts
# print(len(a.without_aparature_counts))
# print(len(a.counts))
# # a.two_theta_in_counts
...
#     r2 = 1 - ss_res / ss_tot
#     return r2,fit_y
# def curve_fitting(count, x,peaks):
...
#         sigma_guess = 1.0
#         try:
#             popt, _ = curve_fit(gaussian, x_fit, y_fit, p0=[A_guess, x0_guess, sigma_guess], bounds=([0.0, x0_guess - 5.0, 0.01],[np.max(count) * 5, x0_guess + 5.0, 10.0]))
#             fitted_params.append(popt)
#             rr2,fit_y = r_2(x_fit,y_fit,popt)
#             r2_list.append(rr2)
#         except RuntimeError:
#             #print(f" Nie udało się dopasować Gaussa do piku przy x = {x[pk]:.2f}")
#             fitted_params.append((np.nan, np.nan, np.nan))
#             r2_list.append(np.nan)
#     return r2_list,popt,fit_y
# @njit
# def peak_detect(count, x, heigh, dist, prom, two_theta, eps, theta_start, step,peaks, plotting=False):
#     if len(peaks) == 0:
#         # print(" Nie znaleziono pików - spróbuj obniżyć progu 'height' lub 'prominence'.")
#         return False
...
#     r2_list,popt,fit_y = curve_fitting(count, x,peaks)
#         # ogranicz dane do małego okna wokół piku
...
# peaks = finding_peaks(a.without_aparature_counts,0, 10, 0.001)
# peak_detect(a.without_aparature_counts, a.x,0, 10, 0.001 ,a.two_theta,3, a.theta_start, a.step,peaks, plotting=False)
...
def peak_detect(count, x, heigh, dist, prom, two_theta, eps, theta_start, step, plotting=False):
    ...
    return len(peaks),len(correct_peaks)/len(two_theta),heigh,dist,prom
# peak_detect(a.counts,a.x,10,20,0.2,a.two_theta,4,a.theta_start,a.step)
# peak_detect(a.without_aparature_counts,a.x,0,10,0.001,a.two_theta,4,a.theta_start,a.step)
